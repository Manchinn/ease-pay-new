    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-[200]" onClick={onClose}>
            <div className="bg-white rounded-2xl relative w-full max-w-[400px] max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                <button className="absolute top-3 right-3 w-8 h-8 bg-white/90 border-none rounded-full text-base cursor-pointer z-10 flex items-center justify-center" onClick={onClose}>✕</button>
                <h2 className="text-xl font-bold mb-1 text-[#0A47CB]">เอกสารที่ต้องเตรียม</h2>
                <p className="text-sm text-slate-500 mb-6">สำหรับ{activeTab === 'edc' ? 'การสมัครใช้ EDC' : 'ลิงก์ชำระออนไลน์'}</p>

                {/* Tabs */}
                <div className="flex bg-slate-100 p-1 rounded-lg mb-6">
                    <button
                        className={`flex-1 py-1.5 px-3 rounded-md text-sm font-medium border-none cursor-pointer transition-all ${docType === 'corporate' ? 'bg-white text-[#4064FF] shadow-sm' : 'text-slate-500 bg-transparent'}`}
                        onClick={() => setDocType('corporate')}
                    >
                        นิติบุคคล
                    </button>
                    <button
                        className={`flex-1 py-1.5 px-3 rounded-md text-sm font-medium border-none cursor-pointer transition-all ${docType === 'individual' ? 'bg-white text-[#4064FF] shadow-sm' : 'text-slate-500 bg-transparent'}`}
                ...
                {docType === 'corporate' && (
                    <ul className="list-none space-y-3 mb-6">
                        ...
                        <li className="text-sm">
                            ...
                            <div className="ml-6 mt-1 text-xs text-[#5E6368]">- กรณีเจ้าของเป็นชาวต่างชาติ: แนบใบอนุญาตทำงานในไทย</div>
                        </li>
                        ...
                        <li className="text-sm">
                            ...
                            <div className="ml-6 mt-1 text-xs text-[#5E6368]">- ใบเสร็จค่าสาธารณูปโภค</div>
                            <div className="ml-6 text-xs text-[#5E6368]">- สัญญาเช่า/ทะเบียนบ้าน/บิลค่าไฟ</div>
                        </li>
                        ...
                        <li className="text-sm">
                            ...
                            <div className="ml-6 mt-1 text-xs text-[#5E6368]">- รายงานการประชุม + หนังสือมอบอำนาจโอนเงิน</div>
                            <div className="ml-6 text-xs text-[#5E6368]">- สำเนาบัตรประชาชนผู้รับมอบ</div>
                        </li>
                        <li className="text-sm">
                            <div className="flex items-start gap-2"><span className="text-orange-500">📋</span> กรณีมอบอำนาจให้บุคคลที่ 3</div>
                            <div className="ml-6 mt-1 text-xs text-[#5E6368]">- หนังสือมอบอำนาจ + ค่าธรรมเนียม 30 บาท</div>
                        </li>
                    </ul>
                )}
                ...
                {docType === 'individual' && (
                    <ul className="list-none space-y-3 mb-6">
                        <li className="text-sm">
                            <div className="flex items-start gap-2"><span className="text-green-500">✅</span> สำเนาบัตรประชาชน หรือ พาสปอร์ต</div>
                            <div className="ml-6 mt-1 text-xs text-[#5E6368]">- แนบรูปถ่ายพร้อมบัตร</div>
                            <div className="ml-6 text-xs text-[#5E6368]">- ชาวต่างชาติ: แนบใบอนุญาตทำงานในไทย</div>
                        </li>
                        <li className="text-sm flex items-start gap-2"><span className="text-green-500">✅</span> สมุดบัญชีเงินฝาก (รับเงินโอน)</li>
                        <li className="text-sm">
                            <div className="flex items-start gap-2"><span className="text-green-500">✅</span> เอกสารยืนยันที่อยู่</div>
                        ...
                        <li className="text-sm flex items-start gap-2"><span className="text-green-500">✅</span> รูปถ่ายป้ายชื่อหน้าร้าน + รูปถ่ายหน้าร้าน + สินค้าในร้าน</li>
                        <li className="text-sm">
                            <div className="flex items-start gap-2"><span className="text-orange-500">📋</span> กรณีโอนเข้าบัญชีบุคคลที่ 3</div>
                            <div className="ml-6 mt-1 text-xs text-[#5E6368]">- หนังสือมอบอำนาจ + สำเนาบัตรผู้รับมอบ</div>
                        </li>
                        <li className="text-sm flex items-start gap-2"><span className="text-green-500">✅</span> ข้อมูลสภาพของร้านค้า</li>
                    </ul>
                )}
