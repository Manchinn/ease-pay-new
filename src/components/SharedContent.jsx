...
export default function SharedContent({ onRegisterClick }) {
        ...
        <>
            ...
            <section className="px-4 pb-8 bg-gradient-to-b from-white to-gray-100">
                <div className="bg-white rounded-2xl p-6 mb-4 shadow-[0_2px_8px_rgba(0,0,0,0.05)]">
                    <h3 className="text-[22px] font-bold mb-3">รู้หรือไม่?</h3>
                ...
                <div className="relative rounded-2xl overflow-hidden">
                    ...
                    <div className="absolute top-0 right-0 bottom-0 w-1/2 bg-gradient-to-r from-transparent to-[rgba(30,41,59,0.6)] flex flex-col justify-center items-end p-4 text-right">
                        <h4 className="text-white text-xl font-bold mb-2">ให้ทุกการขาย<br />เกิดได้ง่ายขึ้น</h4>
                        <p className="text-white text-sm mb-3">ด้วยระบบจ่ายเงิน<br />ที่ใช่กว่า เร็วกว่า</p>
                        <button type="button" className="bg-[#4064FF] text-white border-none rounded-full px-5 py-2 text-base font-semibold cursor-pointer hover:bg-[#3050DD] transition-colors" onClick={onRegisterClick}>เริ่มใช้งานฟรี</button>
                    </div>
                </div>
            </section>
            ...
            <section className="py-8 px-4 bg-white">
                <h2 className="text-xl font-bold mb-4 text-left">คำถามที่พบบ่อย FAQ</h2>
                <div className="flex flex-col gap-3">
                    {faqItems.map((item, i) => (
                        <div key={i} className="border border-slate-200 rounded-xl overflow-hidden">
                            <button type="button" className="w-full bg-white border-none p-4 text-left text-[15px] font-medium cursor-pointer flex justify-between items-center" onClick={() => setOpenFaq(openFaq === i ? null : i)} aria-expanded={openFaq === i} aria-controls={`faq-answer-${i}`} id={`faq-question-${i}`}>
                                {item.q}
                                <span className={`text-[12px] text-slate-500 transition-transform duration-200 ${openFaq === i ? 'rotate-180' : ''}`} aria-hidden>▼</span>
                            </button>
                            {openFaq === i && <div id={`faq-answer-${i}`} className="px-4 pb-4 text-[15px] text-slate-500" role="region" aria-labelledby={`faq-question-${i}`}>{item.a}</div>}
                        </div>
                    ))}
                </div>
            </section>
